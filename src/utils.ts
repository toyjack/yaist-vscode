...
const idsData = require("../data/cjkvi_ids.json");
...
export function getIDS(char: string) {
  if (idsData[char] === undefined) {
    return "X";
  } else {
    return idsData[char];
  }
...
export function getStandard(char: string) {
  if (variants[char] === undefined) {
    return "X";
  } else {
    return variants[char].join("");
  }
}

export function getXmlString(hanzi: string, template: string) {
  let result = template;
  let xmlUnicode = char2Unicode(hanzi).replace("U+", "u");

  result = result.replaceAll("[[unicode]]", xmlUnicode);
  result = result.replaceAll("[[IDS]]", getIDS(hanzi));
  result = result.replaceAll("[[hanzi]]", hanzi);
  result = result.replaceAll("[[GlyphWikiPNG]]", getGwPngUrl(hanzi));
  result = result.replaceAll("[[standard]]", getStandard(hanzi));
  result = result.replaceAll("[[GlyphWikiSVG", getGwSvgUrl(hanzi));
